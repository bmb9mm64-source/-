...
全50ミッション、敵13種、難易度3段階。インストール不要で、PC でもスマホでも遊べます。
...
- **全50ミッション** — M1〜M16 は手設計、M17〜M50 は生成器がつくり、全条件を機械検証しています。
  生成面は**8種類の地形の型**（回廊・四部屋・柱林・中央要塞・蛇行・堀・檻・広間）を巡回するので、面ごとに顔が変わります
- **敵13種** — 固定砲台／遊撃／狙撃／地雷敷設／反射砲台／追跡／多重反射／盾持ち／連射／榴弾／偏差射撃／突進／反射装甲。
...
- **3つの遊び方** — キャンペーン（M1 から順に）／**タイムアタック**（到達済みの1面を残機1で選んで遊ぶ）／
  **サバイバル**（全50面をシャッフルして残機1で連続。どこまで行けるか）
- **実績10種** — 未達成でも条件を隠さないので、次に何をすればよいかが分かります
- **チュートリアル3面** — 「まっすぐ撃つ → 壁の裏を撃つ → 跳ね返りに気をつけろ」の順で跳弾を教えます
- **スマホの縦持ち対応** — 盤面の下に操作帯を出して画面を使い切ります（横持ちなら盤面がさらに大きく）
...
- **569件の単体テスト**。うち全50ミッションについて「開幕に射線が通らない」「跳弾で倒せる位置が必ず存在する」
  といった成立条件を機械検証しています
