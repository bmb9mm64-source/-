...
export const BALANCE = {
  ...
  TURRET: {
    // 固定砲台に共通の内部調整値（GDD §6 v0.23。挙動の見た目に関わらない実装上の刻み）
    // 跳弾狙撃の反射点探索は1回あたり最大4候補×2本の射線判定＝重い処理なので、
    // 毎フレームではなくこの間隔で計算し直し、間は前回の結果を使い回す。
    // 0.08s は 60fps で約5フレーム。標的が 120px/s で動いても位置のずれは約10px、
    // 200px 先なら約3°＝発射許容角（0.15rad≒8.6°）の半分以下に収まる。
    RICOCHET_RECALC_INTERVAL: 0.08, // 反射点を計算し直す間隔 [s]
    RICOCHET_RETARGET_DIST: 64, // 標的がこれだけ動いたら間隔を待たず計算し直す [px]（標的の切り替え対策）
  },
  ...
  BGM: {
    ...
    //
    // v0.21：「長く聴いていられる心地よい曲」への作り替え。
    // v0.20.1 は明るくなったが、旋律が **1小節（8音）のパターンの繰り返し** だったため
    // 数十秒で飽きる／耳に付く曲だった。ゲーム音楽の定石に沿って次の6点を直した：
    //   ① 旋律を「4小節＝32音のフレーズ」単位で書き、A→B→A→C の16小節形式にする
    //      （ループは30秒〜2分が目安。フレーズが変わるので同じ旋律が続かない）
    //   ② 音の長さを揃えない（HOLD で伸ばす）。長短の緩急が「歌っている」感じを生む
    //   ③ 強弱を付ける（拍頭は強く、それ以外は弱く）。同じ音量が続くと耳が疲れる
    //   ④ リードの主波形を矩形波→**三角波**に（矩形波は高次倍音が多く長時間で耳に刺さる）。
    //      チップチューンらしさは矩形波を小さく重ねて確保する
    //   ⑤ 旋律が休んでいる間だけアルペジオを鳴らす（コール＆レスポンス。隙間を埋めつつ濁らせない）
    //   ⑥ スウィング（裏拍を少し後ろへ）で機械的な等間隔を崩す
    // 最後のフレーズ C は主和音へ戻って終わる＝ループの継ぎ目が目立たない。
    ROOT_HZ: 220.0, // 基準音 A3。音程はすべてここからの半音数で表す
    STEPS_PER_BAR: 8, // 1小節あたりのステップ数（8分音符）
    BARS_PER_PHRASE: 4, // 1フレーズの長さ（小節）
    BARS_PER_LOOP: 16, // ループの長さ（小節）＝4フレーズぶん
    SCHEDULE_AHEAD: 0.25, // 先読みして予約する時間 [s]
    TICK_MS: 40, // スケジューラの起動間隔 [ms]

    // 各パートの相対音量（BGM_MASTER に対する比）
    LEAD_VOL: 0.3,
    LEAD_SOFT: 0.68, // 拍頭以外の音量比（強弱＝聴き疲れを防ぐ）
    ARP_VOL: 0.11,
    PAD_VOL: 0.15,
    BASS_VOL: 0.7,
    KICK_VOL: 0.72,
    SNARE_VOL: 0.24,
    HAT_VOL: 0.11,
    HAT_SOFT: 0.55, // 裏拍のハイハットは弱く（一定音量だとカチカチ耳に付く）

    LEAD_WAVE: "triangle" as OscillatorType, // 主波形＝三角波（柔らかい。長時間でも刺さらない）
    LEAD_EDGE_WAVE: "square" as OscillatorType, // 芯を足す副波形（チップチューンらしさ）
    LEAD_EDGE_MIX: 0.28, // 副波形の混ぜ量（上げるほど硬く・派手になる）
    LEAD_DETUNE: 5, // 主波形2基をわずかにずらして厚みを出す [cent]
    LEAD_ATTACK: 0.012, // 立ち上がり [s]（0 だとプチッと鳴る）
    LEAD_GATE: 0.86, // 音符の長さに対する実際の発音長の比（隙間が「歌の息継ぎ」になる）
    LEAD_MAX_DUR: 1.6, // 1音の最長 [s]（伸ばしすぎて濁らせない）
    LEAD_CUT0: 3600, // ローパス開度（発音直後）[Hz]
    LEAD_CUT1: 1500, // 減衰後 [Hz]
    ARP_DUR: 0.14,
    ARP_CUT: 2600,
    BASS_DUR: 0.4,
    BASS_CUT: 520, // ベースのローパス [Hz]
    PAD_ATTACK: 0.35, // パッドの立ち上がり [s]（ゆっくり出るほど柔らかい）
    KICK_F0: 140,
    KICK_F1: 48,
    KICK_DUR: 0.15,
    SNARE_DUR: 0.12,
    ...
    DELAY_FEEDBACK: 0.18, // 浅め（深いと不穏な広がりになる）
    DELAY_MIX: 0.14,

    // シーンごとの曲（GDD §9）。数値は基準音 A3 からの半音数。-99 は休符、-98 は前の音を伸ばす。
    // PROG＝小節ごとのコード（ROOT＝根音、TONES＝構成音）。4小節で一巡する。
    // PHRASES＝4小節（32音）の旋律。C メジャーの**絶対音程**で書く。
    // FORM＝PHRASES の並べ方（16小節ぶん）。A→B→A→C が基本形。
    // BASS＝各コードの根音からの相対音程（1小節ぶん）。
    TRACKS: {
      title: {
        // 軽快なジャズ／ラウンジ調（v0.22）。和音は循環進行 C6 → Am7 → Dm7 → G7
        // （I→vi→ii→V＝ジャズの定番の並び。ジャンルの定石であって特定の曲の写しではない）。
        // 最後が G7＝ドミナント（次へ進みたくなる和音）なので、ループの頭へ自然に戻る。
        TEMPO: 100,
        SWING: 0.22, // 強めのスウィング＝跳ねるリズム。ジャズらしさの半分はこれで決まる
        PROG: [
          { ROOT: 3, TONES: [0, 4, 7, 9] }, // C6
          { ROOT: 0, TONES: [0, 3, 7, 10] }, // Am7
          { ROOT: 5, TONES: [0, 3, 7, 10] }, // Dm7
          { ROOT: -2, TONES: [0, 4, 7, 10] }, // G7
        ],
        PHRASES: [
          // A（問いかけ）：中音域をゆっくり上下する
          [
            19, -98, 22, -98, 24, -98, -98, -99,
            22, -98, 19, -98, 17, -98, -98, -99,
            15, -98, 17, -98, 19, -98, -98, -98,
            17, -98, -98, -98, -99, -99, -99, -99,
          ],
          // B（応え）：1オクターブ上へ持ち上げて表情を変える
          [
            27, -98, 24, -98, 22, -98, -98, -99,
            26, -98, 22, -98, 19, -98, -98, -99,
            24, -98, 22, -98, 24, -98, 26, -98,
            27, -98, -98, -98, -98, -99, -99, -99,
          ],
          // C（結び）：下降して主音 C で終わる＝ループの継ぎ目が目立たない
          [
            22, -98, 24, -98, 27, -98, -98, -99,
            26, -98, 24, -98, 22, -98, 19, -98,
            17, -98, 19, -98, 17, -98, 15, -98,
            15, -98, -98, -98, -98, -98, -98, -98,
          ],
        ],
        FORM: [0, 1, 0, 2],
        BASS: [0, -99, -99, 0, 2, -99, -99, -99], // 根音と第5音（数値は TONES の添字）
        DRUMS: null, // ドラムなし（静かな曲）
        ARP: true,
      },
      game: {
        // 軽快なジャズ／ラウンジ調で、かつ緊張感を保つ（v0.22）。
        // 進行は Am7 → Dm7 → G7 → C6（vi→ii→V→I の循環進行＝ジャズの定石）。
        // マイナーの和音から始めて張りを残しつつ、V→I で軽快に着地する。
        // 音は C メジャー音階の中だけで完結する（＝音階外の濁った音は足さない）。
        TEMPO: 144,
        SWING: 0.22, // 跳ねるリズム。速めのテンポと合わさって「軽快だが落ち着かない」感じになる
        PROG: [
          { ROOT: 0, TONES: [0, 3, 7, 10] }, // Am7
          { ROOT: 5, TONES: [0, 3, 7, 10] }, // Dm7
          { ROOT: -2, TONES: [0, 4, 7, 10] }, // G7
        ...
        PHRASES: [
          // A（主題）：裏拍から入る短い音を混ぜ、跳ねる感じを出す
          [
            24, -99, 24, 27, -98, 26, -98, 24,
            22, -99, 20, 22, -98, 24, -98, -99,
            26, -99, 24, 22, -98, 20, -98, 19,
            15, -98, -98, -99, 19, -98, 22, -98,
          ],
          // B（展開）：高音域へ上げて動きを増やす
          [
            27, -99, 29, 31, -98, 29, -98, 27,
            26, -99, 24, 26, -98, 27, -98, -99,
            29, -99, 27, 26, -98, 24, -98, 22,
            24, -98, -98, -99, 27, -98, -98, -98,
          ],
          // C（結び）：駆け上がってから C6 の構成音（E）へ軽く着地する
          [
            24, 26, 27, -99, 26, 24, -98, -99,
            22, 24, 26, -99, 24, 22, -98, -99,
            20, 22, 24, -99, 26, -98, 27, -98,
            24, -98, -98, 22, -98, 19, -98, -98,
          ],
        ],
        FORM: [0, 1, 0, 2],
        // ウォーキングベース：拍ごとに根音→第3音→第5音→第7音と歩いて上がる。
        // 数値は TONES の添字なので、どのコードでも必ずその和音の構成音になる。
        BASS: [0, -99, 1, -99, 2, -99, 3, -99],
        DRUMS: {
          KICK: [0, 5], // 1拍目と、3拍目の裏（軽く押す）
          SNARE: [2, 6], // 2・4拍目のバックビート
        },
        ARP: true,
      },
      editor: {
        // 静かで作業の邪魔をしない。C → F → C → G
        TEMPO: 78,
        SWING: 0.12,
        PROG: [
        ...
        PHRASES: [
          // A：2分音符ぶんの長い音だけ。ほとんど余白
          [
            15, -98, -98, -98, 19, -98, -98, -98,
            20, -98, -98, -98, 17, -98, -98, -98,
            19, -98, -98, -98, 15, -98, -98, -98,
            14, -98, -98, -98, -98, -99, -99, -99,
          ],
          // B：少し高い位置へ移して気分を変える
          [
            24, -98, -98, -98, 22, -98, -98, -98,
            20, -98, -98, -98, 24, -98, -98, -98,
            27, -98, -98, -98, -98, -98, 24, -98,
            22, -98, -98, -98, -98, -99, -99, -99,
          ],
          // C：ゆっくり降りて主音の上で止まる
          [
            19, -98, -98, 17, -98, -98, 15, -98,
            17, -98, -98, 19, -98, -98, 20, -98,
            19, -98, -98, -98, -98, -98, -98, -98,
            22, -98, -98, -98, -98, -98, -98, -98,
          ],
        ],
        FORM: [0, 1, 0, 2],
        BASS: [0, -99, -99, -99, 2, -99, -99, -99], // 根音と第5音（数値は TONES の添字）
        DRUMS: null, // ドラムなし（静かな曲）
        ARP: true,
      },
    },
