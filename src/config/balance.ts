...
export const BALANCE = {
  ...
  BGM: {
    // 自作合成ループ（GDD §9 v0.11／v0.20 で作り込みを厚くした）。
    // 外部音源ファイルは一切使わない（知財ポリシー）。原作の旋律も使わない＝完全オリジナル。
    //
    // v0.20 の要点：8音のパターンが延々と繰り返されるだけで単調だったので、
    //   ① コード進行（和音の移り変わり）を持たせる
    //   ② 8小節ループにして前半4小節／後半4小節で表情を変える
    //   ③ パッド（和音の敷物）とドラムを足して層を厚くする
    //   ④ リード（旋律）にディレイ（山びこ）を掛けて空間を出す
    ...
    LEAD_DUR: 0.17, // リード1音の長さ [s]
    LEAD_DETUNE: 7, // リードは2基の鋸波をわずかにずらして厚みを出す [cent]
    LEAD_CUT0: 2600, // リードのローパス開度（発音直後）[Hz]
    LEAD_CUT1: 700, // 減衰後 [Hz]
    BASS_DUR: 0.42,
    BASS_CUT: 420, // ベースのローパス（低音を丸くして輪郭を出しすぎない）[Hz]
    PAD_ATTACK: 0.25, // パッドの立ち上がり [s]（ふわっと入る）
    KICK_F0: 130, // キックの開始周波数 [Hz]
    KICK_F1: 45,
    KICK_DUR: 0.16,
    SNARE_DUR: 0.13,
    SNARE_BAND: 1900, // スネアのバンドパス中心 [Hz]
    HAT_DUR: 0.028,
    HAT_HIGHPASS: 7000, // ハイハットのハイパス [Hz]
    DELAY_STEPS: 3, // リードのディレイ時間（8分音符いくつぶんか）
    DELAY_FEEDBACK: 0.32,
    DELAY_MIX: 0.3,

    TRACKS: {
      // シーンごとの曲調（GDD §9）。数値は基準音からの半音数。-99 は休符。
      // PROG＝小節ごとのコード（根音の半音数と構成音）。LEAD/BASS は 1小節8ステップぶん。
      title: {
        // ゆったり。Am → F → C → G（内省的だが暗すぎない進行）
        TEMPO: 84,
        PROG: [0, -4, 3, -2],
        CHORD: [0, 3, 7, 10], // 7th を混ぜて柔らかく
        LEAD_A: [12, -99, 15, -99, 19, -99, 15, 14],
        LEAD_B: [19, -99, 22, 19, 15, -99, 12, -99],
        BASS: [0, -99, 0, -99, 7, -99, 0, -99],
        DRUMS: false,
      },
      game: {
        // 緊張感。Am → F → G → E（最後を E にして次の周へ強く戻す）
        TEMPO: 118,
        PROG: [0, -4, -2, 4],
        CHORD: [0, 3, 7, 10],
        LEAD_A: [12, 12, 15, -99, 19, -99, 15, 12],
        LEAD_B: [24, -99, 22, 19, 15, 19, 22, -99],
        BASS: [0, 0, -99, 0, 7, -99, 0, 3],
        DRUMS: true,
      },
      editor: {
        // 静か。Am → C（作業の邪魔をしない2コード）
        TEMPO: 72,
        PROG: [0, 3, 0, -4],
        CHORD: [0, 7, 10, 14],
        LEAD_A: [12, -99, -99, 15, -99, -99, 19, -99],
        LEAD_B: [15, -99, -99, 12, -99, -99, 10, -99],
        BASS: [0, -99, -99, -99, -99, -99, -99, -99],
        DRUMS: false,
      },
    },
  },
