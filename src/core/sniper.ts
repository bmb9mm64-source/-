 ...
 *   - 発射間隔 平均 4.0±1.0 s
 *   - 跳弾狙撃（直接射線が塞がれているときの外周壁1回反射）の抽選確率 35%
 * Phaser 非依存の純粋 TS。乱数は Rng を注入してテスト可能にする。
 */
import { BALANCE } from "../config/balance";
import { type Bullet, liveBulletCount, SNIPER_BULLET_CFG, spawnBullet } from "./bullet";
...
export interface SniperTank extends TankBody {
  ...
  ricochetMode: boolean; // 抽選に当たり跳弾狙撃を試みているか（GDD §6 v0.6：35%）
}

/** 次回発射間隔（平均±ゆらぎ）を引く */
export function sniperNextInterval(rng: Rng): number {
  const c = BALANCE.SNIPER;
  return c.FIRE_INTERVAL_MEAN + randRange(rng, -c.FIRE_INTERVAL_VAR, c.FIRE_INTERVAL_VAR);
}

/** スナイパーを生成する（初期は下向き） */
export function createSniper(x: number, y: number, rng: Rng): SniperTank {
  return {
    kind: "sniper",
    x,
    y,
    ...
    alive: true,
    state: "IDLE",
    fireTimer: sniperNextInterval(rng),
 ...
 * 跳弾狙撃：直接射線が塞がれているとき、AIM 突入ごとに1回だけ抽選（35%）し、
...
export function updateSniper(e: SniperTank, dt: number, ctx: SniperUpdateContext): void {
  ...
  switch (e.state) {
    ...
    case "AIM": {
      ...
      if (ready) {
        spawnBullet(ctx.bullets, e, e.turretAngle, SNIPER_BULLET_CFG); // 340px/s（GDD §6 v0.6）
        e.fireTimer = sniperNextInterval(ctx.rng);
        e.state = "RELOAD";
